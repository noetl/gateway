...
impl SessionCache {
    ...
    pub async fn connect(&self, nats_url: &str) -> anyhow::Result<bool> {
        ...
        let client = if let Ok(url) = url::Url::parse(nats_url) {
            ...
            let result = if !url.username().is_empty() {
                ...
                tracing::debug!(
                    "Session cache: connecting to NATS with auth: {} (user: {})",
                    server_addr,
                    user
                );
            ...
            match result {
                Ok(c) => c,
                Err(e) => {
                    tracing::warn!("Session cache: failed to connect to NATS ({}), caching disabled", e);
                    return Ok(false);
                }
            }
        } else {
                ...
                match serde_json::from_slice::<CachedSession>(&entry) {
                    Ok(session) => {
                        ...
                        if session.is_active {
                            ...
                            tracing::debug!(
                                "Session cache HIT but inactive: {}",
    ...
    pub async fn invalidate(&self, session_token: &str) -> anyhow::Result<()> {
        ...
        tracing::debug!("Session invalidated: {}", &session_token[..8.min(session_token.len())]);
